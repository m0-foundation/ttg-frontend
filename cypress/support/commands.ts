...
Cypress.Commands.add('checkAppNavigation', () => {
  ...
  cy.get(navbarList).contains('a', 'Actors').and('have.attr', 'href', '/actors')

  cy.get(navbarList)
    .contains('a', 'Proposals')
    .and('have.attr', 'href', '/history')

  cy.get(navbarList).contains('a', 'Config').and('have.attr', 'href', '/config')
})
...
Cypress.Commands.add('clickPreviewProposal', () => {
  cy.get("[data-test='create-proposal-button-preview']").should('exist').click()
})
